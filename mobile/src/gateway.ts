...
export async function getSessionProfile(): Promise<MobileSessionProfile> {
  ...
  const { data, error } = await supabase
    .from('users')
    .select('id,full_name,username,email,role,branch_id,is_active')
    .eq('id', userId)
    .maybeSingle();
  ...
  let permissions: MobilePermission[] = [];
  if (isSuperAdmin) {
    ...
    const { data: roleRow } = await supabase
      ...
      .eq('is_active', true)
      .maybeSingle();
    const raw = Array.isArray(roleRow?.permissions) ? roleRow.permissions : [];
    permissions = raw.filter((value): value is MobilePermission =>
      typeof value === 'string' && MOBILE_PERMISSIONS.includes(value as MobilePermission),
    );
  }
  ...
  const { data: accessRows } = await supabase
    ...
    .select('branch_id')
    .eq('user_id', userId);
  for (const row of (accessRows || []) as { branch_id?: string | null }[]) {
...
export async function getBranches(): Promise<MobileBranch[]> {
  const { data, error } = await supabase
    ...
    .order('name');
  if (error) throw error;
  return ((data || []) as { id: string; name: string; name_en?: string | null }[]).map((row) => ({
    id: row.id,
    name: row.name,
    nameEn: row.name_en,
  }));
...
export async function getBranchPosConfig(branchId: string): Promise<BranchPosConfig> {
  const { data: branch } = await supabase
    ...
    .select('tax_enabled,tax_rate,currency')
    .eq('branch_id', branchId)
    .maybeSingle();
  if (branch) {
    return {
      taxEnabled: Boolean(branch.tax_enabled),
      taxRate: Number(branch.tax_rate || 0),
      currency: String(branch.currency || 'EGP'),
    };
  }
  const { data: global } = await supabase.from('settings').select('tax_enabled,tax_rate,currency').maybeSingle();
  return {
    taxEnabled: Boolean(global?.tax_enabled),
    taxRate: Number(global?.tax_rate || 0),
    currency: String(global?.currency || 'EGP'),
  };
}
...
export async function getDiningTables(branchId: string): Promise<DiningTableSummary[]> {
  const { data: tableRows, error } = await supabase
    .from('dining_tables')
    .select('id,branch_id,area_id,name,status,capacity,area:dining_areas(name)')
    .eq('branch_id', branchId)
    .order('name');
  if (error) throw error;
  ...
  const { data: orderRows } = await supabase
    .from('orders')
    .select('id,order_number,table_id,guest_count,cashier_id,status')
    .eq('branch_id', branchId)
    .in('status', ['open', 'held'])
    .not('table_id', 'is', null);

  const activeOrders = (orderRows || []) as {
    id: string;
    order_number: string;
  ...
  if (cashierIds.length > 0) {
    ...
    for (const row of (users || []) as { id: string; full_name?: string | null; username?: string | null }[]) {
      userNames.set(row.id, row.full_name || row.username || 'مستخدم');
    }
  }
  const orderByTable = new Map(activeOrders.filter((row) => row.table_id).map((row) => [row.table_id as string, row]));

  return ((tableRows || []) as Array<{
    id: string;
    branch_id: string;
    area_id?: string | null;
    name: string;
    ...
    return {
      ...
      name: row.name,
      status: order || row.status === 'occupied' ? 'occupied' : 'available',
      guestCount: order?.guest_count || null,
      capacity: row.capacity || null,
      activeOrderId: order?.id || null,
      activeOrderNumber: order?.order_number || null,
...
export async function getCatalog(branchId: string): Promise<WaiterCatalogProduct[]> {
  const { data, error } = await supabase
    .from('products')
    .select('id,branch_id,category_id,name,name_en,description,image_url,sale_price,is_active,category:categories(name)')
    .eq('branch_id', branchId)
    .eq('is_active', true)
    .order('name');
  if (error) throw error;
...
export async function getProductModifiers(productId: string): Promise<ModifierGroup[]> {
  ...
  return (result.groups || []).map((group) => ({
    ...
    maxSelections: Math.max(1, Number(group.max_selections || 1)),
...
const rpcItems = (items: WaiterCartItemInput[]) => items.map((item) => ({
...
export async function updateTableOrder(orderId: string, items: WaiterCartItemInput[], config: BranchPosConfig): Promise<void> {
  const { data: order, error: orderError } = await supabase
    .from('orders')
    .select('order_type,table_id,customer_id,guest_count,notes,discount_amount,discount_type')
    .eq('id', orderId)
    .maybeSingle();
  ...
  const { data, error } = await supabase.rpc('update_order', {
    ...
    p_guest_count: order.guest_count || null,
    p_notes: order.notes || null,
    p_items: rpcItems(items),
    p_subtotal: totals.subtotal,
    p_discount_amount: discountAmount,
    p_discount_type: order.discount_type || 'amount',
    p_tax_amount: taxAmount,
    p_total: totalAfterDiscount + taxAmount,
    p_status: 'open',
  });
  if (error) throw error;
...
export async function getMyOrders(branchId?: string): Promise<WaiterOrderSummary[]> {
  ...
  let query = supabase
    .from('orders')
    .select('id,order_number,branch_id,table_id,total,status,kitchen_status,kitchen_sent_at,cashier_id,table:dining_tables(name)')
  ...
  return ((data || []) as Array<{
    ...
    table?: { name?: string | null } | { name?: string | null }[] | null;
  }>).map((row) => {
    ...
    return {
      ...
      waiterName: profile.displayName,
      total: Number(row.total || 0),
      status: row.status,
      kitchenStatus: row.kitchen_status,
      sentToKitchenAt: row.kitchen_sent_at,
    };
  });
}
...
export async function getOrderDetails(orderId: string): Promise<WaiterOrderDetails> {
  const { data: order, error: orderError } = await supabase
    .from('orders')
    .select('id,order_number,branch_id,table_id,total,status,kitchen_status,kitchen_sent_at,guest_count,notes,cashier_id,table:dining_tables(name)')
    .eq('id', orderId)
    .maybeSingle();
  if (orderError || !order) throw orderError || new Error('ORDER_NOT_FOUND');
  ...
  const { data: itemRows, error: itemError } = await supabase
    .from('order_items')
    .select('product_id,quantity,unit_price,modifier_option_ids,notes,product:products(name)')
    .eq('order_id', orderId);
  if (itemError) throw itemError;

  const { data: waiter } = order.cashier_id
    ? await supabase.from('users').select('full_name,username').eq('id', order.cashier_id).maybeSingle()
    : { data: null };
  ...
  return {
    ...
    tableName: table?.name || '—',
    waiterName: waiter?.full_name || waiter?.username || '—',
    total: Number(order.total || 0),
    status: order.status,
    kitchenStatus: order.kitchen_status,
    sentToKitchenAt: order.kitchen_sent_at,
    guestCount: order.guest_count || null,
    ...
    items: ((itemRows || []) as Array<{
      ...
      return {
        ...
        modifierOptionIds: Array.isArray(row.modifier_option_ids) ? row.modifier_option_ids : [],
