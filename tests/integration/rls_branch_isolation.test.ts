...
describe.skipIf(skip)('RLS branch isolation', () => {
  ...
  describe('SELECT isolation (branch-scoped tables)', () => {
    for (const tbl of SPEC) t(`${tbl.name}: staff see only their branch, admins see all`, () => selectBattery(tbl));
  });

  describe('Write isolation (branch-scoped tables)', () => {
    for (const tbl of SPEC) t(`${tbl.name}: writes are branch/role gated (${tbl.mode})`, () => writeBattery(tbl));
  });
  ...
  describe('branch_settings (per-branch settings)', () => {
    ...
    const tmpBranch = async () => {
      const orgId = (await client.query<{ organization_id: string }>(`SELECT organization_id FROM public.branches WHERE id = $1`, [ids.branchB])).rows[0].organization_id;
      return (await client.query<{ id: string }>(`INSERT INTO public.branches (name, organization_id) VALUES ('tmp', $1) RETURNING id`, [orgId])).rows[0].id;
    };
    const tmpRow = async () => {
  ...
  describe('RBAC hardening (044)', () => {
    t('REVOKE: _post_journal_entry and log_audit_action are internal-only', async () => {
      const codeA = (await client.query<{ code: string }>(`SELECT code FROM public.chart_of_accounts WHERE id = $1`, [ids.coaCashA])).rows[0].code;
      const payload = JSON.stringify([{ account_code: codeA, debit: 100, credit: 0 }, { account_code: codeA, debit: 0, credit: 100 }]);
      ...
      await runProbe(client, 'log_audit_action admin', adminId(), `SELECT public.log_audit_action(NULL::uuid, 'x', 'x', NULL::uuid, '{}'::jsonb)`, 'denied');
      await runProbe(client, 'log_audit_action cashier', cashierId(), `SELECT public.log_audit_action(NULL::uuid, 'x', 'x', NULL::uuid, '{}'::jsonb)`, 'denied');
    });
    t('get_audit_trail: audit.view required; otherwise empty', async () => {
      const direct = await runProbe(client, 'audit_log SELECT cashier own', cashierId(), `SELECT count(*)::int AS c FROM public.audit_log WHERE branch_id = $1`, 'ok', [ids.branchA]);
      expect(Number(direct.rows[0].c)).toBeGreaterThanOrEqual(1);
      const parseTrail = (v: unknown): unknown[] => typeof v === 'string' ? JSON.parse(v) : (v as unknown[]);
      const admin = await runProbe(client, 'get_audit_trail admin', adminId(), `SELECT public.get_audit_trail($1) AS trail`, 'ok', [ids.branchA]);
      expect(parseTrail(admin.rows[0].trail).length).toBeGreaterThanOrEqual(1);
      const cashier = await runProbe(client, 'get_audit_trail cashier', cashierId(), `SELECT public.get_audit_trail($1) AS trail`, 'ok', [ids.branchA]);
    ...
    t('sale financial writes require process_sale instead of direct DML', async () => {
      const ins = (discount: number, branch: string) => `INSERT INTO public.sales (invoice_number, branch_id, warehouse_id, subtotal, discount_amount, tax_amount, total, paid_amount, payment_method, status) VALUES ('${uniq('INV')}', '${branch}', '${ids.whA}', 100, ${discount}, 0, 100, 100, 'cash', 'completed')`;
    ...
    t('record_login_failure: does not extend an in-force lock (044)', async () => {
      ...
      await client.query(`UPDATE public.users SET failed_attempts = 4, is_locked = true, lock_until = now() + interval '5 minutes' WHERE id = $1`, [uid]);
      const before = await client.query<{ failed_attempts: number; lock_until: string }>(`SELECT failed_attempts, lock_until FROM public.users WHERE id = $1`, [uid]);
      ...
      const after = await client.query<{ failed_attempts: number; lock_until: string }>(`SELECT failed_attempts, lock_until FROM public.users WHERE id = $1`, [uid]);
      ...
      await client.query(`UPDATE public.users SET failed_attempts = 0, is_locked = false, lock_until = NULL WHERE id = $1`, [uid]);
      await client.query(`SELECT public.record_login_failure('ca')`);
      const inc = await client.query<{ failed_attempts: number }>(`SELECT failed_attempts FROM public.users WHERE id = $1`, [uid]);
    ...
    t('guard_role_permissions: branch managers cannot mint admin-only roles (044)', async () => {
      const ins = (perms: string) => `INSERT INTO public.roles (role, name_ar, name_en, permissions) VALUES ('${uniq('RG')}', 'X', 'Y', '${perms}'::jsonb)`;
