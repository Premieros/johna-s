...
describe('print execution truth contract', () => {
  ...
  it('records receipt submission only after Electron or Print Agent accepts execution', () => {
    ...
    expect(receiptBuilder).toContain('const template = buildReceiptFixedTemplate(receipt, s, lang, isAr);');
    expect(receiptBuilder).toContain('template,');
    expect(receiptBuilder).toContain('buildFixedThermalTemplateHtml(template)');
  ...
  it('treats missing or rejecting local Print Agent as failure, never success', () => {
    ...
    const drawer = between(localAgent, 'export async function executeCashDrawerKick', 'export async function printKitchenStationsLocally');
