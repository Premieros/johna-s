-- Fix shift closing report runtime error:
--   column "display_name" does not exist
-- display_name is a JSON key, not a SQL column in the aggregate scope.
-- Keep the public RPC contract unchanged and order by the real user expression.
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'account_id', t.id, 'account_name', t.account_name,
    'opening_balance', t.opening_balance,
    'gl_balance', round(t.opening_balance + COALESCE(sum(l.debit - l.credit), 0), 2)
