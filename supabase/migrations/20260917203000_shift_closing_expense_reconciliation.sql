-- Reconcile shift-closing expenses and net revenue.
-- New expenses are linked directly by shift_id. Historical posted expenses created
-- before the shift_id contract are included only when they belong to the same
-- branch and were created inside the shift's open/close time window.
-- This avoids losing historical expenses while preventing double counting.
...
DECLARE
  ...
  v_legacy_cash_expenses numeric := 0;
  v_net numeric := 0;
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(sum(s.total - COALESCE(s.refunded_amount, 0)), 0)
  ...
  WHERE e.status = 'posted'
    ...
    AND (
      e.shift_id = p_shift_id
      OR (
        e.shift_id IS NULL
        AND e.created_at >= v_shift.opened_at
        AND e.created_at <= COALESCE(v_shift.closed_at, now())
      )
    );
  ...
  SELECT round(
    ...
    + COALESCE(sum(CASE
        WHEN COALESCE(payment_method, 'cash') = 'cash'
             AND operation_type IN ('sale', 'cash_in') THEN amount
        WHEN COALESCE(payment_method, 'cash') = 'cash'
             AND operation_type IN ('refund', 'expense', 'cash_out') THEN -amount
        ELSE 0
      END), 0),
    2
  )
  ...
  v_expected := round(v_expected - v_legacy_cash_expenses, 2);
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'user_id', u.id,
    'display_name', COALESCE(u.full_name, u.email),
  ...
  FROM (
    ...
    user_sales AS (
      SELECT
        COALESCE(sa.created_by, s.cashier_id) AS user_id,
  ...
  RETURN jsonb_build_object(
    ...
    'actual_cash', v_shift.actual_amount,
