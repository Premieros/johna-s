-- Stabilization: POS kitchen send is allowed to oversell raw material stock.
-- Manufactured inventory units that are auto-produced for a POS sale must obey
-- the same raw-stock rule. Manual production remains strict.
...
DECLARE
  ...
  v_allow_negative_raw boolean := COALESCE(p_notes, '') = 'AUTO_SALE_PRODUCTION';
BEGIN
