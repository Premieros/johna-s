...
-- 1) Every branch has one protected default area named "Main Area".
-- 2) Main Area owns the canonical fixed tables "Table 01".."Table 50".
-- 3) Custom areas stay user-managed and may contain custom tables.
-- 4) Canonical floor-plan RPCs cannot add/move/rename tables in ways that break Main Area.
...
BEGIN
  FOR v_branch IN SELECT id FROM public.branches LOOP
    ...
    LOOP
      IF v_row.name ~ '^طاولة [0-9]{2}$' OR v_row.name ~ '^Table [0-9]{2}$' THEN
  ...
  IF TG_OP = 'INSERT' AND v_new_default THEN
    IF NEW.name !~ '^Table (0[1-9]|[1-4][0-9]|50)$'
      ...
      RAISE EXCEPTION USING
        ...
        DETAIL = 'Main Area accepts only active Table 01..Table 50.';
    END IF;
    RETURN NEW;
  END IF;
...
REVOKE ALL ON FUNCTION public.floor_plan_add_table(uuid,text,integer,uuid,text,jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.floor_plan_add_table(uuid,text,integer,uuid,text,jsonb) TO authenticated, service_role;

REVOKE ALL ON FUNCTION public.floor_plan_update_table(uuid,text,integer,uuid,text,jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.floor_plan_update_table(uuid,text,integer,uuid,text,jsonb) TO authenticated, service_role;
