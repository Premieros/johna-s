-- Production patch: historical backfill must never mutate or delete
-- FIFO debt/settlement state that was created by live traffic before the run.
BEGIN;
...
CREATE INDEX IF NOT EXISTS idx_raw_fifo_debts_backfill_run
  ON public.raw_fifo_debts(backfill_run_id)
