...
import {
  buildKitchenFixedTemplate,
  buildStationTicketText,
...
export interface CloudPrintPayload { text?: string; html?: string; template?: FixedThermalTemplate; paperWidthMm?: number; copies?: number; }
...
export async function enqueueCloudKitchenPrintJobs(params: { branchId: string; items: KitchenSendItem[]; context: LocalKitchenPrintContext; paperWidthMm?: number }) {
  ...
  const results = await Promise.all(entries.map(async ([station, stationItems]) => {
    ...
    const payload = {
      ...
      template: buildKitchenFixedTemplate(station, stationItems, params.context, paperWidthMm),
      paperWidthMm,
      copies: 1,
    };
