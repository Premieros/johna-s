...
export const trade = {
  ...
  updatePurchase(p: { p_purchase_id: string; p_supplier_id: string; p_warehouse_id: string | null; p_subtotal: number; p_discount_amount: number; p_tax_amount: number; p_total: number; p_paid_amount: number; p_payment_method: string; p_notes: string | null; p_items: PurchaseItemInput[] }): ApiResult<RpcResult & { previous_purchase_id?: string; previous_revision_number?: string }> { return rpc('update_purchase_invoice', p); },
