...
function originAllowed(origin) {
  ...
  try { const u = new URL(origin); return ['127.0.0.1','localhost'].includes(u.hostname) && ['http:','https:'].includes(u.protocol); }
  catch { return false; }
}
...
function ps(script, envOverrides = {}) {
  return new Promise((resolve,reject)=>{
    execFile('powershell.exe',['-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-Command',script],{
      ...
      if(err){const d=String(stderr||err.message||'').trim();if(err.killed||err.signal)return reject(new Error(`POWERSHELL_TIMEOUT${d?`: ${d}`:''}`));return reject(new Error(d||'POWERSHELL_FAILED'));}
      resolve(stdout);
    });
  });
}
...
async function printTextOnce(printerName,text){
  ...
  try{
    await ps("$p=$env:JOHNS_PRINTER;$f=$env:JOHNS_PRINT_FILE;Get-Content -LiteralPath $f -Raw -Encoding UTF8 | Out-Printer -Name $p",{JOHNS_PRINTER:printerName,JOHNS_PRINT_FILE:tmp});
...
async function kickDrawerOnce(printerName){
  ...
  try{
    await ps("$p=$env:JOHNS_PRINTER;$f=$env:JOHNS_PRINT_FILE;Get-Content -LiteralPath $f -Encoding Byte -Raw | Out-Printer -Name $p",{JOHNS_PRINTER:printerName,JOHNS_PRINT_FILE:tmp});
  }finally{try{fs.unlinkSync(tmp);}catch{}}
}
...
async function runPrintJob({jobId,printer,text}){cleanupDedupe();if(jobId&&completedJobs.has(jobId))return{deduplicated:true};if(jobId&&inFlightJobs.has(jobId)){await inFlightJobs.get(jobId);return{deduplicated:true};}const task=enqueuePrinter(printer,()=>withRetry(()=>printTextOnce(printer,text),`print:${printer}`));if(jobId)inFlightJobs.set(jobId,task);try{await task;if(jobId)completedJobs.set(jobId,Date.now()+DEDUPE_TTL_MS);return{deduplicated:false};}finally{if(jobId&&inFlightJobs.get(jobId)===task)inFlightJobs.delete(jobId);}}
